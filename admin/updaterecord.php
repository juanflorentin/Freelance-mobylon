...
$targetaction=$_GET['tgact'];			
		
			
			//===Deleting property listing
			if($targetaction=='editjobcat')
			{
				
				$id=$_GET['id'];
				$name=$_POST['editname'];
				$ssql="Update tbljobcategories set name='$name' where id=$id";
				$sql = mysql_query($ssql) or die
							(mysql_error()); 
							
				header("Location:jobcategories.php?edit=tru");
			}		//===Deleting property listing
			if($targetaction=='editcountry')
			{
				
				$id=$_GET['id'];
				$name=$_POST['editname'];
				$ssql="Update tblcountry set name='$name'  where id=$id";
				$sql = mysql_query($ssql) or die
							(mysql_error()); 
							
				header("Location:country.php?edit=tru");
			}	
			
			
			
			if($targetaction=='editexpdate')
			{
				
				$id=$_GET['id'];
				$expdate=$_POST['expdate'];
				$ssql="Update tbljobs set expirdate='$expdate'  where id=$id";
				$sql = mysql_query($ssql) or die
							(mysql_error()); 
							
				header("Location:managelisting.php?edit=tru");
			}	
			...
			if($targetaction=='dellisting')
			{
				...
				$id=$_GET['listingid'];
				$mid=$_GET['mid'];
				$ssql="delete from tbllisting  where id=$id";
				$sql = mysql_query($ssql) or die
							(mysql_error()); 
							
				header("Location:managelisting.php?del=tru&mid=".$mid);
			}	
			...
			if($targetaction=='deleteavatar')
			{
				
				$id=$_GET['cid'];
				// deleting the avatar file
				$sqqq=mysql_fetch_array(mysql_query("select avatar from tblavatar where avid='$id'"));
				$avaa="../files/avatars/".$sqqq['avatar'];
				unlink($avaa);
				
				$ssql="delete from tblavatar where avid=$id";
				$sql = mysql_query($ssql) or die
							(mysql_error()); 
					
				header("Location:avatars.php?del=tru");
			}	
